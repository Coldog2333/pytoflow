...
def fast_imgs2video(imgdir, fps=24, music_name=None, output_video='', thread=1):
    if music_name:
...
def fast_extract_video(video_name, save_dir='', extract_music=False, music_name=''):
    ...
    command = 'ffmpeg' + \
              ' -i ' + video_name + \
              ' -q:v 2 ' + \
              ' -f image2 %s/%%06.png' % save_dir + \
              ' -s %d*%d' % (width, height) + \
              ' -r %d' % fps
    print(command)
    flag = subprocess.call(command)    # ffmpeg -i F:\TOFlow\test.mp4 -f image2 F:\TOFlow\frame\test\%06d.png -s 1920*1080 -r 24
    if not flag:
