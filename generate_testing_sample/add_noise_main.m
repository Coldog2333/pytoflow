...
noise_path = '/home/ftp/Coldog/Dataset/toflow/vimeo_septuplet/sequences_with_noise';
...
for i=1:length(videos)
    ...
    for j=1:length(seps)
        ...
        add_noise_to_input(fullfile(data_path, videos(i).name, seps(j).name), fullfile(noise_path, videos(i).name, seps(j).name))
        count = count + 1;
        if mod(count,500)==0
